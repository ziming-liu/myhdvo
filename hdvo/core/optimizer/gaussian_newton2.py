...
class GaussianNewton2(Optimizer):
    ...
    def __init__(self, params, lr=0.15, betas=(0.9, 0.999), eps=1e-4,
        ...
        self.lam = lam
        super(GaussianNewton2, self).__init__(params, defaults)
    ...
    def get_trace(self, params, grads):
        ...
        hvs = []
        for i, grad in enumerate(grads):
            param_size = grad.shape
            grad = grad.reshape((-1, 1))
            vi = v[i].reshape((-1,1))
            tmp = torch.einsum('ij,ji,ij->ij',grad, grad.T, vi)
            tmp = tmp.reshape(param_size)
            hvs.append(tmp)
        ...
        for hv in hvs:
            ...
            if len(param_size) <= 2:  # for 0/1/2D tensor
                ...
                tmp_output = hv.abs()
            elif len(param_size) ==3: # 3D tensor
                tmp_output = hv.abs()
                ...
                tmp_output = torch.mean(hv.abs(), dim=[2,3,4], keepdim=True)
            # L-M method, + lambda * I
            tmp_output = tmp_output + torch.ones_like(tmp_output, device=tmp_output.device) * self.lam # lambda * I 
