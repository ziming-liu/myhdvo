...
class GaussianNewton5(Optimizer):
    """Implements GaussianNewton5 algorithm. J*J * J_l 
    It has been proposed in `GaussianNewton5: An Adaptive Second Order Optimizer for Machine Learning`.
    Arguments:
    ...
    def __init__(self, params, lr=0.15, betas=(0.9, 0.999), eps=1e-4,
        ...
        super(GaussianNewton5, self).__init__(params, defaults)
    ...
    def step(self, l1_grads=None, l2_grads=None, step_loss=None, closure=None):
        ...
        for (p, group, l1_grad, l2_grad, hut_trace) in zip(params, groups, l1_grads, l2_grads, hut_traces):
            ...
            if len(state) == 0:
                ...
                state['exp_avg'] = torch.zeros((p.data.reshape(-1).shape[0], 1)).cuda()
                # Exponential moving average of Hessian diagonal square values
                state['exp_hessian_diag_sq'] = torch.zeros((p.data.reshape(-1).shape[0], p.data.reshape(-1).shape[0])).cuda()
            ...
            #B = grad * step_loss
            #print("B ", B)
            hut_trace = torch.linalg.inv( l1_grad.reshape(-1,1) @ l1_grad.reshape(-1,1).T + self.lam * torch.eye(l1_grad.reshape(-1).shape[0]).cuda() )

            exp_avg.mul_(beta1).add_(l2_grad.detach_().reshape(-1,1), alpha=1 - beta1)
            ...
            # make update
            p.data = p.data - \
                group['lr'] * ( (denom @ exp_avg).reshape(p.data.shape) / bias_correction1  + group['weight_decay'] * p.data)
