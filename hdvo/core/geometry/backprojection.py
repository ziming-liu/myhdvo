...
class Backprojection(nn.Module):
    ...
    def __init__(self, height, width):
        ...
        meshgrid = np.meshgrid(range(self.width), range(self.height), indexing='xy') # xv(h,w), yv(h,w)
        id_coords = np.stack(meshgrid, axis=0).astype(np.float32) # 2xhxw
        ...
        self.xy = torch.unsqueeze(
                        ...
                        , 0) # 1x2xhw
        self.xy = torch.cat([self.xy, self.ones], 1) # 1x3xhw
        self.xy = nn.Parameter(self.xy, requires_grad=False) # to be trainabel
    ...
    def forward(self, depth, inv_K, img_like_out=False):
        ...
        xy = self.xy.repeat(depth.shape[0], 1, 1) # Nx3xhw
        ones = self.ones.repeat(depth.shape[0],1,1)  # Nx1xhw
