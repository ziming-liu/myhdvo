...
LastEditTime: 2023-12-28 22:27:30
Description: ...
...
class PSMNet(nn.Module):
    ...
    def forward(self, *input):
        ...
        if self.with_cp:
            ...
            stack_out = self._forward(stack_input)
        l_fms = stack_out[:B, ...]
        r_fms = stack_out[B:, ...]
...
class PSMNetSingle(nn.Module):
    ...
    def __init__(self, in_planes=3, out_dim=32, batch_norm=True, pretrained=None, align_corners=False, with_cp=False):
        super(PSMNetSingle, self).__init__()
        self.in_planes = in_planes
        self.batch_norm = batch_norm
        self.pretrained = pretrained
        self.align_corners = align_corners
        ...
        self.branch1 = nn.Sequential(
            nn.AvgPool2d((64, 64), stride=(64, 64)),
            nn.Conv2d( 128, 32, 1, 1, 0, 1, bias=False),
            nn.ReLU()
        )
        self.branch2 = nn.Sequential(
            nn.AvgPool2d((32, 32), stride=(32, 32)),
        ...
        self.lastconv = nn.Sequential(
            conv_bn_relu(batch_norm, 320, 128, 3, 1, 1, 1, bias=False),
            nn.Conv2d(128, out_dim, kernel_size=1, padding=0, stride=1, dilation=1, bias=False)
        )
        self.init_weights()
...
@BACKBONES.register_module()
class PSMNetOld(PSMNetSingle):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.branch1 = nn.Sequential(
            nn.AvgPool2d((64, 64), stride=(64, 64)),
            conv_bn_relu(True, 128, 32, 1, 1, 0, 1, bias=False),
             
        )


@BACKBONES.register_module()
class PSMNetUnshared(nn.Module):
    """
    Backbone proposed in PSMNet.
    Args:
        in_planes (int): the channels of input
    ...
    def __init__(self, **kwargs):
        super(PSMNetUnshared, self).__init__()
        self.left_branch = PSMNetSingle(**kwargs)
        self.right_branch = PSMNetSingle(**kwargs)
