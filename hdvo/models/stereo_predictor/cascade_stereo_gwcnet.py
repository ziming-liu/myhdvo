...
import math
import time
from ..utils.cascade_stereo_submodule import *
...
class CascadeStereoGwcNet(nn.Module):
    def __init__(self, maxdisp, ndisps, disp_interval_pixel, using_ns, ns_size, grad_method="detach",
                 cr_base_chs=[32, 32, 32], **kwargs):
        super(CascadeStereoGwcNet, self).__init__()
    ...
    def forward_train(self, left_image, right_image, **kwargs):
        ...
        outputs = self._forward(left_image, right_image)
        disp_gt = kwargs["left_disps"].reshape(outputs["stage1"]["pred"].shape)
        ...
        loss = stereo_psmnet_loss(outputs, disp_gt, mask, dlossw=[float(e) for e in dlossw.split(",") if e])
    ...
    def forward_test(self, left_image ,right_image, **kwargs):
        ...
        outs = [[],[],[],[],[],[]]
        outs[0] = outputs[f"stage{self.num_stage}"]["pred"].detach().cpu().numpy()
