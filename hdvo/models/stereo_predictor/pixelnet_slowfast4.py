...
class PixelNetSlowFast4(nn.Module):
    ...
    """
    def __init__(self, backbone, losses, neck_dense, neck_sparse, maskIV=False, cls_losses=None, alpha=1, normalize=True, sample_rate=4,
                  dense_channels=8, sparse_channels=64, max_disp=192, predict_format="depth", 
                  img_pad_zeros=False, fast_image_volume=True, 
                  pretrained=None, mask_left=False,
                  use_stereo_loss = False,
                  use_depth_loss = False,
                   use_multiscale_test=False, multiscales= [(256, 832), (384,1280), (576, 1920)], **kwargs):
        """
        disp_l1_loss e.g.
        [dict(
        ...
        self.use_stereo_loss = use_stereo_loss
        self.use_multiscale_test = use_multiscale_test
        ...
        if cls_losses is not None:
            self.cls_loss_func = build_loss(cls_losses)
        else:
            self.cls_loss_func = None
        ...
        if self.use_stereo_loss:
            self.linear_right = nn.Sequential(nn.Conv3d(1, 1, kernel_size=3, padding=1),
                                            nn.BatchNorm3d(1),
    ...
    def build_image_volume_roll(self, leftImage, rightImage, max_disp=192, mask_left=False, img_pad_zeros=False):
        ...
        #B, C, H, W = leftImage.shape
        #D = max_disp
        if self.mask_template is None or (self.mask_template.shape[0],self.mask_template.shape[-2],self.mask_template.shape[-1])!=(leftImage.shape[0],leftImage.shape[-2],leftImage.shape[-1]):
    ...
    def build_image_volume_mat(self, leftImage, rightImage, max_disp=192, mask_left=False, 
        ...
        if self.maskIV and self.mask_template is not None:
    ...
    def left_volume_to_right_volume(self, left_volume, ):
        device = left_volume.device
        B, C, D, H, W = left_volume.shape
        
        if self.right_indices is None or self.right_indices.shape != (D,W):
            right_indices = torch.arange(W,device=device).view((1,W)).repeat((D,1))
            shifts = torch.arange(D,device=device).reshape(D,1)
            self.right_indices = (right_indices + shifts) % W # left + shift = right
        #left_volume = left_volume.unsqueeze(2).expand(B, C, D, H, W)
        right_volume = torch.gather(left_volume, 4, self.right_indices.reshape((1,1,D,1,W)).expand(B,C,D,H,W))
        #for i in range(D): 
        #    vis_depth_tensor(left_volume[:1,0,i,...].detach().clone(), "/home/ziliu/vis/left2rightIV/", f"{i}_leftvolume")
        #    vis_depth_tensor(right_volume[:1,0,i,...].detach().clone(), "/home/ziliu/vis/left2rightIV/", f"{i}_rightvolume")
        #exit()
        if self.right_mask_template is None or (self.right_mask_template.shape[0],self.right_mask_template.shape[-2],self.right_mask_template.shape[-1])!=(left_volume.shape[0],left_volume.shape[-2],left_volume.shape[-1]):
        #    print("re-init mask template")
            #print("leftImage.shape: ", leftImage.shape)
            #print("mask_template.shape: ", self.mask_template.shape if self.mask_template is not None else "None")
            self.right_mask_template = torch.ones(B, 1, D, H, W, device=left_volume.device)
            for i in range(D):
                if i==0:
                    continue
                self.right_mask_template[:, :, i, :, D-i:D] = 0
            #right_volume = right_volume.mul_(self.right_mask_template.detach())
        return right_volume #.mul_(self.right_mask_template.detach())
    ...
    def forward_train(self, left_imgs, right_imgs, **kwargs):
        ...
        if self.fast_image_volume:
            image_volume = self.build_image_volume_mat(leftImage, rightImage, self.max_disp)
        else:
        ...
        if "left_gt_disp" in self.data[0].keys() and self.use_stereo_loss:
            right_cost_full = self.left_volume_to_right_volume(cost_full.unsqueeze(1))
            #right_cost_full = self.linear_right(right_cost_full)
            right_disp = self.disp_predictor([right_cost_full.squeeze(1)])[0]
        ...
        if "left_gt_disp" in self.data[0].keys():
            gt_disp = self.data[0]["left_gt_disp"] # torch.stack([self.data[ti]["left_gt_disp"] for ti in range(self.num_frames)], 1).reshape(self.bs*self.num_frames, 1, self.height, self.width)
            if self.use_depth_loss:
                loss_left_gt_depth = self.loss(self.disp2depth_factor.reshape(self.bs,1,1,1)/ disp, self.disp2depth_factor.reshape(self.bs,1,1,1)/ gt_disp)
                losses.update(loss_left_gt_depth)
                #print("loss_left_gt_depth: ", loss_left_gt_depth)
            else:
                loss_left_gt_disp = self.loss(disp, gt_disp)
                losses.update(loss_left_gt_disp)

            #if self.cls_loss_func is not None:
            #    loss_cls = self.cls_loss_func(cost_full, gt_disp)
            #    losses.update(loss_cls)
        if "right_gt_disp" in self.data[0].keys() and self.use_stereo_loss:
            gt_disp = self.data[0]["right_gt_disp"] # torch.stack([self.data[ti]["left_gt_disp"] for ti in range(self.num_frames)], 1).reshape(self.bs*self.num_frames, 1, self.height, self.width)
            loss_right_gt_disp = self.loss(disp, gt_disp)
            for k,v in loss_right_gt_disp.items():
                losses[k+"_right"] = v
    ...
    def forward_test(self, left_imgs, right_imgs, **kwargs):
        ...
        testing."""
        if self.use_multiscale_test:
            return self._do_test_multiscale(left_imgs, right_imgs,**kwargs)
    ...
    def _do_test(self, left_imgs, right_imgs,**kwargs):
        ...
        # build image volume 
        if self.fast_image_volume:
            image_volume = self.build_image_volume_mat(leftImage, rightImage)
        else:
            image_volume = self.build_image_volume(leftImage, rightImage)
        # sample 
        image_volume_sampled = F.interpolate(image_volume.contiguous(),scale_factor=(1/self.sample_rate,1,1), mode='nearest')
        feature_sparse, feature_dense = self.backbone(image_volume_sampled)
        cost_sparse, cost_dense = self.neck_sparse(feature_sparse)[0], self.neck_dense(feature_dense)[0] # B 256 D/sample_rate H/4 W/4 
        cost_sparse = F.interpolate(cost_sparse, scale_factor=(self.max_disp//cost_sparse.shape[2],1,1), mode='trilinear')
        cost_dense = F.interpolate(cost_dense, scale_factor=(self.max_disp//cost_dense.shape[2],1,1), mode='trilinear')
        ...
        t1 = time.time()
        try:
            outputs[6] = [ item["left_frame_paths"][0] for item in kwargs["img_metas"] ]
        except:
            pass
        #print("time for inference: ", t1-t0)
        return outputs
        
    def _do_test_multiscale(self, left_imgs, right_imgs,**kwargs):
        """Defines the computation performed at every call when training."""
        t0=time.time()
        self._init_data(left_imgs, right_imgs, **kwargs)
        t1 = time.time()
        ...
        network_pred_right_disparities = None
        ##print("estimate depth in test stage...")
        leftImage_origin = left_imgs #left_imgs.reshape((-1, self.channel,self.height,self.width))
        rightImage_origin = right_imgs #right_imgs.reshape((-1, self.channel,self.height,self.width))

        multiscale_leftImage = [F.interpolate(leftImage_origin, size=scale, mode='bilinear') for scale in self.multiscales]
        multiscale_rightImage = [F.interpolate(rightImage_origin, size=scale, mode='bilinear') for scale in self.multiscales]
        multiscale_disp_preds = []
        multiscale_depth_preds = []
        for leftImage, rightImage, scale in zip(multiscale_leftImage, multiscale_rightImage, self.multiscales):
            t0 = time.time()
            # build image volume 
            if self.fast_image_volume:
                image_volume = self.build_image_volume_mat(leftImage, rightImage)
            ...
            ##print(f"rank {rank} >> pred disp  {pred_disparities[0].shape}")
            self.num_level = len(pred_disparities)
            pred_H, pred_W = pred_disparities[0].shape[-2:]
            multiscale_disp_preds.append(F.interpolate(pred_disparities[0], size=(self.height,self.width), mode='bilinear') * (pred_W/self.width) )
            if self.predict_format == "depth":
                multiscale_depth_preds.append( torch.stack([self.disp2depth_factor[bi]/(1e-10+multiscale_disp_preds[-1].reshape((self.bs,self.num_frames,1,pred_H,pred_W))[bi,0,:,:,:]) for bi in range(self.bs)]))
         
        if self.predict_format == "depth":
            
            fused_depth = torch.mean(torch.stack(multiscale_depth_preds, dim=0), dim=0)
            all_pred_depthes = fused_depth.float().cpu().numpy()
            outputs[0]=all_pred_depthes # batch, views, h, w
        elif self.predict_format == "disparity":
            fused_disp = torch.mean(torch.stack(multiscale_disp_preds, dim=0), dim=0)
            predisps = fused_disp.float().cpu().numpy()
            outputs[0]=predisps # batch, views, h, w
        else:
            raise ValueError
    ...
    def cls_loss(self, pred, gt, **kwargs):
        losses = {}
        losses.update(self.cls_loss_func(pred, gt))
        return losses
