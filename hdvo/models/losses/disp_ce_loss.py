...
LastEditTime: 2023-04-25 19:15:37
...
class DispCELoss(object):
    ...
    def __init__(self, max_disp, start_disp=0, weight=1, weights=None,
    ...
    def loss_per_level(self, estDisp, gtDisp):
        ...
        scaled_gtDisp = torch.round(scaled_gtDisp).squeeze(1).long() # N H W 
        if self.set_range:
            #assert mask.sum() >= 1.0, 'L1 loss: there is no point\'s disparity is in ({},{})! \n >> \n {}'.format(self.start_disp,
            #                                                                            self.max_disp / scale, scaled_gtDisp)
            scaled_gtDisp = scaled_gtDisp.reshape(-1)
            mask = (scaled_gtDisp > self.start_disp) & (scaled_gtDisp < (self.max_disp / scale))
            D = estDisp.shape[1]
            estDisp = estDisp.permute(0,2,3,1).reshape(-1,D)
            estmask = mask.unsqueeze(1).repeat(1,D)
            #print("estDisp ", estDisp)
            #print("scaled_gtDisp ", scaled_gtDisp)
            loss = F.cross_entropy(estDisp[estmask].reshape((-1,D)), scaled_gtDisp[mask])
            #loss = torch.abs(estDisp[mask]-scaled_gtDisp[mask]).mean()
        else:
            loss = F.cross_entropy(estDisp, scaled_gtDisp, )
    ...
    def __call__(self, estDisp, gtDisp, weights=None):
