...
LastEditTime: 2023-10-19 11:34:02
Description: ...
...
class HUBERLoss(nn.Module):
    ...
    def forward(self, pred, target,):
        if self.delta is not None:
            ...
            #if input2.shape[0] <= 1:
            #    input2 = input
            #input = input2.reshape([-1,1])
            thresh = self.MAD(input.detach())
            delta = float(thresh)
        #print("delta: ", delta)
        huberloss = F.huber_loss(pred, target, reduction='none', delta=delta)#float(thresh))
    ...
    def MAD(self, input):
        ...
        ret_mad = sorted_adfrommedian[:, size//2] if size%2 else 0.5*(sorted_adfrommedian[:, size//2-1]+sorted_adfrommedian[:, size//2]) # fix the bug: use int value //2 is not correct, will make the VO results very bad.
        ret_mad = ret_mad.reshape([-1,1])
        # Compute properties
        sigma = (ret_mad * 1.4826) # batchsize x 1
...
def MAD(self, input):
        ...
        assert size > 1 
        # Compute median
        sorted_input, idx = torch.sort(input)
        median = sorted_input[size//2] if size%2 else 0.5*(sorted_input[size//2-1]+sorted_input[size//2])
        #    // For each element, compute absolute deviation (don't care if the order was changed by median)
        adfrommedian = torch.abs(input - median)
        # // Compute median of absolute deviations
        sorted_adfrommedian, idx = torch.sort(adfrommedian)
        ret_mad = sorted_adfrommedian[size//2] if size%2 else 0.5*(sorted_adfrommedian[size//2-1]+sorted_adfrommedian[size//2]) # fix the bug: use int value //2 is not correct, will make the VO results very bad.
        ...
        sigma_minimal = torch.finfo(torch.float64).min
        sigma_maximal = torch.finfo(torch.float64).max
        if sigma < sigma_minimal: 
            sigma = sigma_minimal
        if sigma > sigma_maximal: 
            sigma = sigma_maximal

        thresh = 1.2816 * sigma
        if (thresh < 1e-5): thresh = torch.FloatTensor([1e-5]).type_as(input)
        return thresh
