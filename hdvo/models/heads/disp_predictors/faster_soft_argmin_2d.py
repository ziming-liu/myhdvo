...
class FasterSoftArgmin2D(nn.Module):
    """
    A faster implementation of soft argmin.
    details can refer to modeling.stereo.disp_predictors.soft_argmin
    Args:
    ...
    def __init__(self, max_disp, start_disp=0, dilation=1, alpha=1.0, normalize=True):
        super(FasterSoftArgmin2D, self).__init__()
    ...
    def forward(self, cost_volume, disp_sample=None):
        D_size = cost_volume.shape[1]
        left_disp_map = self.compute_one_disp(cost_volume, disp_sample)
        #left_disp_map = self.compute_one_disp(cost_volume[:,:int(D_size/2), :, :], disp_sample)
        #right_disp_map = self.compute_one_disp(cost_volume[:,int(D_size/2):, :, :], disp_sample)
        
        #assert left_disp_map.shape == right_disp_map.shape
        return left_disp_map #[left_disp_map, right_disp_map] 

    def compute_one_disp(self, cost_volume, disp_sample=None):
        ...
        #print("prob_volume {}".format(prob_volume.shape))
        disp_map = self.disp_regression(prob_volume)
        #print("disp map {}".format(disp_map.shape))
        # [B, 1, 1, W, H] -> [B, 1, W, H]
