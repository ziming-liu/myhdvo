...
LastEditTime: 2023-03-16 11:23:16
...
class CatCostProcessor(CostProcessor):
    ...
    def __init__(self, cost_computation, cost_aggregator):
        '''
        description: 
        return: {*}
        '''        
    ...
    def cat_fms(self, reference_fm, target_fm, max_disp=192, start_disp=0, dilation=1, disp_sample=None):
        """
        ...
        disp_index = torch.linspace(start_disp, end_disp, disp_sample_number).cuda()

        concat_fm = torch.zeros(N, C * 2, disp_sample_number, H, W).cuda() # fix the type bug when using half-float. ziming 21-7-8
        idx = 0
        for i in disp_index:
    ...
    def fast_cat_fms(self, reference_fm, target_fm, max_disp=192, start_disp=0, dilation=1, disp_sample=None):
        device = reference_fm.device
        ...
        if disp_sample is None:
            ...
            disp_sample = disp_sample.view(1, D, 1, 1).expand(B, D, H, W).to(device).type_as(reference_fm) # fix the type bug when using half-float. ziming 21-7-8
    ...
    def forward(self, ref_fms, tgt_fms, disp_sample=None):
        ...
        cat_cost = None
        if self.cat_func == "default":
            cat_cost = self.cat_fms(ref_fms, tgt_fms, disp_sample=disp_sample, **self.default_args)
        elif self.cat_func == "fast":
            cat_cost = self.fast_cat_fms(ref_fms, tgt_fms, disp_sample=disp_sample, **self.default_args)
        else:
            raise ValueError
