...
class StereoHDVOUnsup(HDVO):
    def __init__(self, depth_net=None, 
                 ...
                 **kwargs):
        super(StereoHDVOUnsup, self).__init__(depth_net, segment_net, pose_net, brightness_net, 
    ...
    def forward_train(self, left_imgs, right_imgs, **kwargs):
        ...
        if T==2:
            ...
            for level, ref_disp in enumerate(ref_disps): 
                ...
                # direct head 
                initcTr, pose_6d = self.pose_net(torch.stack([ref_left_imgs, cur_left_imgs],1)) # cTr 
                masks = None #(ref_depth > 1) & (ref_depth < 100)
        ...
        if T==3: 
            ...
            for level, ref_disp in enumerate(ref_disps): 
                ...
                # pre direct head 
                initcTr, pose_6d = self.pose_net(torch.stack([ref_left_imgs, pre_left_imgs],1)) # cTr 
                masks =None # (ref_depth > 1) & (ref_depth < 100)
                ...
                # next direct head 
                initcTr, pose_6d = self.pose_net(torch.stack([ref_left_imgs, next_left_imgs],1)) # cTr 
    ...
    def _do_test(self, left_imgs, right_imgs=None, **kwargs):
        ...
        #masks = (depths > 1) & (depths < 50) # mask out invalid depth
        #masks = None
        depth_masks = (depths > 1) & (depths < 80) # mask out invalid depth
        
        
        edge_mask = torch.zeros_like(depth_masks)
        edge_mask[:,:,int(0.3*H):int(0.9*H),int(0.1*W):int(0.9*W)] = 1 
        masks = depth_masks * edge_mask
        #if self.pred_pose is None:
        #    self.pred_pose = initcTr
        self.pred_pose = initcTr
        #print(pred_pose)
        #pred_pose = self.ddvo_module( Ir=ref_left_imgs, Zr=depths, Ic=cur_left_imgs, \
        #                              K=kwargs["intrinsics"][:,0], imask=masks, cTr=initcTr)
        self.pred_pose = self.ddvo_head(source_img=cur_left_imgs, target_depth=depths,
                                        sTt=self.pred_pose, K=kwargs["intrinsics"][:,0],  target_img=ref_left_imgs,
                                        target_mask=masks,
                                        test_mode=True)
