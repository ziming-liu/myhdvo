...
template <typename scalar_t, typename index_t>
  ...
  __global__ void grid_sampler_2d_fc_kernel(
    ...
    CUDA_KERNEL_LOOP_TYPE(index, nthreads, index_t) {
      ...
      if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
        ...
        scalar_t *oup_ptr_NC = output.data + n * oup_sN;
        index_t NC_offset = n * gInp_sN;
        scalar_t *inp_ptr_NC = input.data + n * inp_sN;
        for (index_t c = 0; c < C; ++c, inp_ptr_NC += inp_sC, NC_offset += gInp_sC, gOut_ptr_NCHW += gOut_sC, oup_ptr_NC += oup_sC) {
          ...
          if (within_bounds_2d(up, left, inp_H, inp_W)) {
            scalar_t up_val = oup_ptr_NC[up * oup_sH + w * oup_sW];
            scalar_t left_val = oup_ptr_NC[h * oup_sH + left * oup_sW];
            giy -= up_val * 0.5 * gOut;
            gix -= left_val * 0.5 * gOut;
          }
          if (within_bounds_2d(down, right, inp_H, inp_W)) {
            scalar_t down_val = oup_ptr_NC[down * oup_sH + w * oup_sW];
            scalar_t right_val = oup_ptr_NC[h * oup_sH + right * oup_sW];
...
std::vector<torch::Tensor> grid_sample2d_cuda_fc(
    ...
    if (count > 0) {
        AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "grid_sampler_2d_grad2_cuda", [&] {
          if (canUse32BitIndexMath(input) && canUse32BitIndexMath(grid) &&
            ...
            grid_sampler_2d_fc_kernel<scalar_t>
