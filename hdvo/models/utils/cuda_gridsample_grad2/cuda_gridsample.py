...
class _GridSample2dBackward(torch.autograd.Function):
    ...
    def forward(ctx, grad_output, input, grid, padding_mode=0, align_corners=True):
        ...
        if _use_pytorch_1_11_api:
            #print(ctx.needs_input_grad[1], ctx.needs_input_grad[2])
            output_mask = (True, True)#(ctx.needs_input_grad[1], ctx.needs_input_grad[2])
            grad_input, grad_grid = op[0](grad_output, input, grid, 0, padding_mode, align_corners, output_mask)
        else:
