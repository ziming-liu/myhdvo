...
import grad2_grid_sample as gridsample_grad2
import esm_grid_sample as gridsample_esm
#gridsample_grad2 = load(name='gridsample_grad2', sources=['hdvo/models/utils/cuda_gridsample_grad2/gridsample_cuda.cpp', 'hdvo/models/utils/cuda_gridsample_grad2/gridsample_cuda.cu'], verbose=True)

#gridsample_esm = load(name='gridsample_esm', sources=['hdvo/models/utils/esm_grid_sample/esm_grid_sample/esm_grid_sample_cuda.cpp', 'hdvo/models/utils/esm_grid_sample/esm_grid_sample/esm_grid_sample_kernel.cu'], verbose=True)

def grid_sample_2d_esm(input, grid, gt_map, padding_mode='zeros', align_corners=True):

    assert padding_mode in ['zeros', 'border']
    return _GridSample2dForward.apply(input, grid, gt_map, padding_mode, align_corners)
...
class _GridSample2dForward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, grid, gt_map, padding_mode=0, align_corners=True):
        assert input.ndim == 4
        assert grid.ndim == 4
        ...
        ctx.save_for_backward(input, grid, gt_map, output)
    ...
    def backward(ctx, grad_output):
        input, grid, gt_map, output = ctx.saved_tensors
        grad_input, grad_grid = _GridSample2dBackward.apply(grad_output, input, grid, gt_map, output, ctx.padding_mode, ctx.align_corners)
        return grad_input, grad_grid, None, None, None # return number should be  same as the input number 

class _GridSample2dBackward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, grad_output, input, grid, gt_map, output, padding_mode=0, align_corners=True):
        #op = torch._C._jit_get_operation('aten::grid_sampler_2d_backward')
        interpolate_mode = 0 #'bilinear'
        input_requires_grad = True
        grad_input, grad_grid = gridsample_esm.grad2_2d_esm(grad_output, input, grid, gt_map, output, \
                                 interpolate_mode, padding_mode, align_corners, input_requires_grad)
    ...
    def backward(ctx, grad2_grad_input, grad2_grad_grid):
        ...
        assert grid.is_cuda 
        assert grad2_grad_grid.is_cuda
        assert grad2_grad_input.is_cuda 
        ...
        return grad_grad_output, grad_input, grad_grid, None, grad_output, None, None # return number should be same as the input number 
