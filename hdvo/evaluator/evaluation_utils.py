...
def read_file_data(files, data_root,cam_id="left"):
    gt_files = []
    gt_calib = []
    im_sizes = []
    im_files = []
    ...
    for filename in files:
        if cam_id =="left":
            filename = filename.split()[0] # 2011_09_26/2011_09_26_drive_0002_sync/image_02/data/0000000069.jpg
        elif cam_id=="right":
        ...
        date = splits[0] # 2011_09_26
        im_id = splits[4][:10] # 0000000069
        file_root = '{}/{}'
        
        im = filename
        if im.split('.')[-1] == 'jpg':
            im = '.'.join([im.split('.')[0], 'png'])
        #vel = '{}/{}/velodyne_points/data/{}.bin'.format(splits[0], splits[1], im_id)
        vel = os.path.join(splits[0],splits[1],'velodyne_points/data','{}.bin'.format(im_id))
    ...
    print('{} files missing'.format(num_probs))
...
def test_calib(calib_dir= '/data/acentauri/user/ziliu/data/kitti_raw_data/2011_09_26/'):
    cam2cam = read_calib_file(os.path.join(calib_dir , 'calib_cam_to_cam.txt'))
    velo2cam = read_calib_file(os.path.join(calib_dir , 'calib_velo_to_cam.txt'))
    print(velo2cam)
    velo2cam = np.hstack((velo2cam['R'].reshape(3,3), velo2cam['T'][..., np.newaxis]))
...
def generate_gt_depth_for_odometry(seq_dir):
    ...
    images = os.listdir(os.path.join(seq_dir, "image_2"))
    ...
    for t_id in tqdm(range(num_samples)):
        ...
        if not os.path.exists(os.path.join(seq_dir, f"depth_{camera_id}")):
            os.makedirs(os.path.join(seq_dir, f"depth_{camera_id}"))
        print(depth)
        depth = depth * 256
        depth = depth.astype(np.uint16)
        cv2.imwrite(os.path.join(seq_dir, f"depth_{camera_id}", "{:0>6}.png".format(t_id)), depth.squeeze())
