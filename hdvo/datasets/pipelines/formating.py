...
class StereoFormatShape:
    ...
    Args:
        input_format (str): Define the final imgs format.
        collapse (bool):  T ==1  Default: False.
    """

    def __init__(self, input_format, keys=[], ):
        self.input_format = input_format
        self.keys = keys
        assert len(keys) > 0
        if self.input_format not in ['NCTHW', 'NTCHW', 'NCHW', 'NCDHW' ]:
            raise ValueError(
                f'The input format {self.input_format} is invalid.')
    ...
    def __call__(self, results):
        ...
        for k in self.keys: 
            if not isinstance(results[k], np.ndarray): # list -> array
                results[k] = np.array(results[k])
            imgs = results[k] 
            if len(imgs.shape)==3: # disp or depth:  TXHXW -> T H W 1 
                imgs = np.expand_dims(imgs, axis=3)
            #assert len(imgs.shape) ==4, "rgb and depth, disp all need to have 3 dims, H W C"

            if self.input_format == 'NTCHW':
                # input is T x H x W x C
                # T x H x W x C -> T C H W 
                imgs = np.transpose(imgs, (0, 3, 1, 2))
            elif self.input_format == 'NCTHW':
                imgs = np.transpose(imgs, (3, 0, 1, 2))
            elif self.input_format == 'NCHW':
                assert imgs.shape[0]==1, "the sequence dim should be 1"
                imgs = imgs.reshape(imgs.shape[1:])
                imgs = np.transpose(imgs, (2,0,1))
            elif self.input_format == 'NCDHW':
                # 1 x D x H x W x C
                assert imgs.shape[0]==1, "the sequence dim should be 1"
                imgs = imgs[0] # 1 x D x H x W x C -> D x H x W x C
                imgs = np.transpose(imgs, (3, 0, 1, 2)) # D x H x W x C -> C x D x H x W
            results[k] = imgs
