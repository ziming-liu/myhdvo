...
class kittiOdomEval():
    ...
    def rotationError(self, pose_error):
        ...
        # modify the bug by ziming and ezio. 
    ...
    def plotPath_2D_3(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        delta_x, delta_y, delta_z = x_gt[0]-x_pred[0], y_gt[0] - y_pred[0], z_gt[0] - z_pred[0]
        x_pred, y_pred, z_pred = x_pred+delta_x, y_pred+delta_y, z_pred+delta_z
        ...
        start_point[0], start_point[1] = x_gt[0], z_gt[0]
        ...
        start_point[0], start_point[1] = x_gt[0], y_gt[0]
        plt.plot(start_point[0], start_point[1], style_O, label='Start Point')
        plt.legend(loc="upper right", prop={'size':fontsize_})
        plt.xlabel('x (m)', fontsize=fontsize_)
        ...
        start_point[0], start_point[1] = y_gt[0], z_gt[0]
        plt.plot(start_point[0], start_point[1], style_O, label='Start Point')
        plt.legend(loc="upper right", prop={'size':fontsize_})
    ...
    def plotPath_3D(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        assert poses_result[0].any() == poses_gt[0].any()
        fig = plt.figure(figsize=(8,8), dpi=110)
        try:
            ax = fig.gca(projection='3d')
        except: # zm fix the bug for high version of matplotlib
            fig = plt.figure()
        ...
        start_point[0], start_point[1], start_point[2] =  plane_point[0,0], plane_point[0,1], plane_point[0,2]
        ...
        for seq in self.eval_seqs:
            ...
            if len(poses_gt) > num_samples:
                def dict_slice(adict, start, end):
                    ...
                    return dict_slice
                poses_gt = dict_slice(poses_gt,0,num_samples)
            ...
            def dump_xyz(source_to_target_transformations):
                ...
                for source_to_target_transformation in source_to_target_transformations:
                    ...
                    xyzs.append(cam_to_world[:3, 3]) # translation
                return xyzs
            ...
            def compute_ate(gtruth_xyz, pred_xyz_o):
                ...
                offset = gtruth_xyz[0] - pred_xyz_o[0] # align the first frame's poses
                pred_xyz = pred_xyz_o + offset[None, :]
                # Optimize the scaling factor
                #print(np.sum(gtruth_xyz * pred_xyz))
                scale = np.sum(gtruth_xyz * pred_xyz) / np.sum(pred_xyz ** 2) if np.sum(pred_xyz ** 2)!=0 else 1
                alignment_error = pred_xyz * scale - gtruth_xyz
                rmse = np.sqrt(np.sum(alignment_error ** 2)) / gtruth_xyz.shape[0]
                return rmse, scale

            pred_poses = []
            for k, v in poses_result.items():
                pred_poses.append(v)
            pred_poses = np.stack(pred_poses)
            pre_local_poses = []
            for i in range(1, len(pred_poses)):
                pre_local_poses.append(
                    np.linalg.inv(np.dot(np.linalg.inv(pred_poses[i - 1]), pred_poses[i]))) 
            
            gt_poses = []
            for k, v in poses_gt.items():
                gt_poses.append(v)
            
            gt_global_poses = np.stack(gt_poses)
            gt_xyzs = gt_global_poses[:, :3, 3]

            gt_local_poses = []
            for i in range(1, len(gt_global_poses)): # from frame 1
                gt_local_poses.append(
                    np.linalg.inv(np.dot(np.linalg.inv(gt_global_poses[i - 1]), gt_global_poses[i]))) # relative  gt pose i-> i-1
            ...
            def compute_RPE(  gt, pred):
                ...
                min_rpe_trans = np.min(np.asarray(trans_errors))
                max_rpe_trans = np.max(np.asarray(trans_errors))
                min_rpe_rot = np.min(np.asarray(rot_errors))
                max_rpe_rot = np.max(np.asarray(rot_errors))
                return rpe_trans, rpe_rot, min_rpe_trans, max_rpe_trans, min_rpe_rot, max_rpe_rot

            rpe_trans,rpe_rot, min_rpe_trans, max_rpe_trans, min_rpe_rot, max_rpe_rot = compute_RPE(gt_global_poses, pred_poses)
 
            print("RPE trans: {:.4f}, min: {:.4f}, max: {:.4f}  \n RPE rot: {:.4f}, min: {:.4f}, max: {:.4f}".format(rpe_trans, min_rpe_trans, max_rpe_trans,
                        rpe_rot,min_rpe_rot, max_rpe_rot))
            ...
            for i in range(0, num_frames):
                local_xyzs = np.array(dump_xyz(pre_local_poses[i:i + track_length])) # input is relative pose 
                gt_local_xyzs = np.array(dump_xyz(gt_local_poses[i:i + track_length]))
                
                rmsei, scale_factor = compute_ate(gt_local_xyzs, local_xyzs)
                ates.append(rmsei)
                scale_factors.append(scale_factor)

            print("\n  abs Trajectory error: {:0.4f}, std: {:0.4f}\n".format(np.mean(ates), np.std(ates)))
            print("average scale factor ", np.mean(scale_factors))


            cha = poses_gt[0] - poses_result[0]
            for idx in range(len(poses_result)):
                poses_result[idx] = poses_result[idx] + cha
            ...
            print ('Distance (m): %d' % self.distance)
            print ('Max speed (km/h): %d' % (self.max_speed*3.6))
            print ("Average sequence translational RMSE (%):   {:.4f}".format(ave_t_err * 100))
            print ("Average sequence rotational error (deg/100m): {:.10f}\n".format(ave_r_err/np.pi * 180 * 100))
            with open(eva_seq_dir + '/%s_stats.txt' % seq, 'w') as f:
                f.writelines('Average sequence translation RMSE (%):    {:.4f}\n'.format(ave_t_err * 100))
                f.writelines('Average sequence rotation error (deg/100m):  {:.4f}'.format(ave_r_err/np.pi * 180*100))
...
if __name__ == '__main__':
    ...
    config = {"gt_dir":args.gt_dir, "result_dir": args.result_dir, "eva_seqs": args.eva_seqs,\
                "toCameraCoord": args.toCameraCoord}
    print(config)
    pose_eval = kittiOdomEval(config)
    pose_eval.eval(toCameraCoord=args.toCameraCoord)   # set the value according to the predicted results

"""  
 python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  /home/ziliu/rv/vkitti2_gt_pose/    --result_dir    /home/ziliu/rv/vkitti2_pred_pose/   --eva_seqs  01_pred  
  python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  /home/ziliu/rv/vkitti2_gt_pose/    --result_dir    /home/ziliu/rv/vkitti2_pred_pose/   --eva_seqs  02_pred
# evaluate  kitti odometry  sequence 09
python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  /data/acentauri/user/ziliu/ckp/mmvo/work_dirs/undepth_kittiodom_tpr_spr_tdr_dense/kittiodometry_gt_pose/    --result_dir    /data/acentauri/user/ziliu/ckp/mmvo/work_dirs/undepth_kittiodom_tpr_spr_tdr_dense/kittiodometry_pred_pose/   --eva_seqs  09_pred

python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  work_dirs/undepth_vkitti2full_spr_sparse_scenflowpretrain/gt_pose    --result_dir    work_dirs/undepth_vkitti2full_spr_sparse_scenflowpretrain/pred_pose   --eva_seqs  20_pred

python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  /data/acentauri/user/ziliu/data/kitti_odometry/pose_GT  --result_dir    /data/acentauri/user/ziliu/ckp/mmvo/work_dirs/undepth_kittiodom_tpr_spr_tdr_dense_scratch/kittiodometry_pred_pose/   --eva_seqs  09_pred,10_pred


python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  /data/acentauri/user/ziliu/data/kitti_odometry/pose_GT  --result_dir    /data/acentauri/user/ziliu/ckp/mmvo/work_dirs/undepth_kittiodom_tpr_spr_tdr_dense_stereopretrain/kittiodometry_pred_pose/   --eva_seqs  09_pred

python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  ./vkitti2/gt_pose  --result_dir    ./vkittti2/pred_pose  --eva_seqs  06_pred


python  KITTI_odometry_evaluation_tool/evaluation.py  --gt_dir  \
     /home/ziliu/rv/vkitti2_gt_pose/   --result_dir  \
    /data/acentauri/user/ziliu/ckp/mmvo/work_dirs/undepth_vkitti2full_spr_dense_scratch/pred_pose/ \
         --eva_seqs  06_pred
"""