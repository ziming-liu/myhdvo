...
class StereoResize:
    ...
    def __call__(self, results):
        ...
        if "left_pred_depths" in results.keys() and results["left_pred_depths"] is not None:
            ...
            results['left_pred_depths'] = [ F.interpolate(torch.FloatTensor(depth.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
                            for depth in results['left_pred_depths'] ]
        if "right_pred_depths" in results.keys()  and results["right_pred_depths"] is not None :
            for depth in results['right_pred_depths']:
                assert len(depth.shape) == 2, "depth has to be loaded with HxW shape"
            results['right_pred_depths'] = [ F.interpolate(torch.FloatTensor(depth.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
                            for depth in results['right_pred_depths'] ]
        if "left_depths" in results.keys() and results["left_depths"] is not None:
            for depth in results['left_depths']:
                assert len(depth.shape) == 2, "depth has to be loaded with HxW shape"
            #print("before >> ", results['left_depths'][0])
            results['left_depths'] = [ F.interpolate(torch.FloatTensor(depth.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
                            for depth in results['left_depths'] ]
        ...
        if "right_depths" in results.keys()  and results["right_depths"] is not None :
            for depth in results['right_depths']:
                assert len(depth.shape) == 2, "depth has to be loaded with HxW shape"
            results['right_depths'] = [ F.interpolate(torch.FloatTensor(depth.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
                            for depth in results['right_depths'] ]
        ...
        if "left_disps" in results.keys() and results["left_disps"] is not None:
            ...
            results['left_disps'] = [self.scale_factor[0] * F.interpolate(torch.FloatTensor(disp.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
            for disp in results['left_disps']
            ]
        if "right_disps" in results.keys()  and results["right_disps"] is not None :
            for disp in results["right_disps"]:
                assert len(disp.shape) == 2, "the disparity has to be loaded with HxW shape"
            results['right_disps'] = [self.scale_factor[0] * F.interpolate(torch.FloatTensor(disp.copy()).unsqueeze(0).unsqueeze(0), size=(new_h, new_w), mode=self.interpolation,align_corners=False).squeeze().numpy()
            for disp in results['right_disps']
            ]
...
class StereoNormalize:
    ...
    def __init__(self, mean, std, to_rgb=True, adjust_magnitude=False):
        if not isinstance(mean, Sequence):
            raise TypeError(
                f'Mean must be list, tuple or np.ndarray, but got {type(mean)}'
            )
    ...
    def __call__(self, results):
        ...
        n = len(results['left_imgs'])
        h, w, c = results['left_imgs'][0].shape
        imgs = np.empty((n, h, w, c), dtype=np.float32)
        for i, img in enumerate(results['left_imgs']):
            imgs[i] = img
        ...
        results['left_imgs'] = imgs
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        

        n = len(results['right_imgs'])
        h, w, c = results['right_imgs'][0].shape
        imgs = np.empty((n, h, w, c), dtype=np.float32)
        for i, img in enumerate(results['right_imgs']):
            imgs[i] = img
...
class StereoResize2(object):
    ...
    def _resize_depth_disp(self, disp, scale, w_scale_factor=1.0):
        ...
        return w_scale_factor * F.interpolate(torch.FloatTensor(disp.copy()).squeeze().unsqueeze(0).unsqueeze(0),
                        size=(scale[1],scale[0]), mode='bilinear',align_corners=False).squeeze().numpy()
...
class StereoPad(object):
    ...
    def __call__(self, results):
        ...
        results["left_imgs"] = [self._pad_img(img) for img in results["left_imgs"]]
        #results['pad_shape'] = results["left_imgs"][0].shape
        results['pad_fixed_size'] = self.size
        results['pad_size_divisor'] = self.size_divisor
        #if "right_imgs" in results.keys():
        results["right_imgs"] = [self._pad_img(img) for img in results["right_imgs"]]
        if "left_depths" in results.keys():
            results["left_depths"] = [self._pad_disp_depth(disp )for disp in results["left_depths"]]
        if "right_depths" in results.keys():
            results["right_depths"] = [self._pad_disp_depth(disp )for disp in results["right_depths"]]
        if "left_disps" in results.keys():
            results["left_disps"] = [self._pad_disp_depth(disp,  )for disp in results["left_disps"]]
        if "right_disps" in results.keys():
            results["right_disps"] = [self._pad_disp_depth(disp,  )for disp in results["right_disps"]]
