...
class PyramidStereoHead2AdabinLightConv(nn.Module):
    def __init__(self, in_channels, max_disp, alpha, normalize, latent_dims=128, 
        ...
        return: {*}
        '''        
        super(PyramidStereoHead2AdabinLightConv, self).__init__()
        ...
        self.stage1 = conv3d_bn_relu(True, in_channels[0], self.latent_dims, 3, 1, 1)
        self.stage2 = conv3d_bn_relu(True, self.latent_dims, self.latent_dims, 3, 1, 1)
    ...
    def cost_matcher(self, raw_costs):
        ...
        prob = self.prob_conv(s2).squeeze(1)

        adabins =  self.adabins_conv(s2).squeeze(1)
...
@HEADS.register_module()
class PyramidStereoHead2AdabinLightConvLargeRange(PyramidStereoHead2AdabinLightConv):
