...
LastEditTime: 2023-03-16 11:23:10
...
class PSMNetHead48New(BaseStereoHead):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48New, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
    ...
    def cost_builder(self, stereo_features):
        ...
        B,_,H,W = ref_fms.shape
        bins = self.disp_sample.reshape(1,self.max_disp,1,1).repeat(B,1,H,W)
        cat_cost = self.fast_cat_fms(ref_fms, tgt_fms, bins, self.max_disp)
