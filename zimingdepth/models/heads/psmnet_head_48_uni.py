...
LastEditTime: 2023-03-30 15:14:55
...
class PSMNetHead48Uni(BaseStereoHead):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, local_predictor=False, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48Uni, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
    ...
    def forward(self, stereo_features):
        ...
        return pred_disps,decoded_features
...
@HEADS.register_module()
class PSMNetHead48UnionelevelMono2StereoRemove(nn.Module):
    def __init__(self, in_channels, max_disp, alpha, normalize, losses=None, learn_resize=None,  **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48UnionelevelMono2StereoRemove, self).__init__()
        self.in_channels = in_channels
    ...
    def cost_builder(self, stereo_features,bins):
        ...
        ref_fms, tgt_fms = stereo_features[0], stereo_features[1]
        #print(ref_fms.shape)
        cat_cost = self.fast_cat_fms(ref_fms, tgt_fms,bins)
        return (cat_cost,)
...
@HEADS.register_module()
class PSMNetHead48Unionelevel(BaseStereoHead):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, learn_resize=None,  **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48Unionelevel, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
        self.in_channels = in_channels
        self.learn_resize = learn_resize
        self.disp_range = disp_range 
        self.start_disp = disp_range[0]
...
@HEADS.register_module()
class PSMNetHead48UnionelevelFPN(PSMNetHead48Unionelevel):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48UnionelevelFPN, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
        self.in_channels = in_channels
        self.disp_range = disp_range 
        self.start_disp = disp_range[0]
        self.max_disp = disp_range[1]
...
@HEADS.register_module()
class PSMNetHead48UnionelevelFPNtreble1D(PSMNetHead48Unionelevel):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48UnionelevelFPNtreble1D, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
        self.in_channels = in_channels
        self.disp_range = disp_range 
        self.start_disp = disp_range[0]
        self.max_disp = disp_range[1]
...
@HEADS.register_module()
class PSMNetHead48UnionelevelFPN2plus1D(PSMNetHead48Unionelevel):
    def __init__(self, in_channels, disp_range, alpha, normalize, losses=None, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48UnionelevelFPN2plus1D, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
        self.in_channels = in_channels
        self.disp_range = disp_range 
        self.start_disp = disp_range[0]
        self.max_disp = disp_range[1]
...
@HEADS.register_module()
class PSMNetHead48UnionelevelPostResize(BaseStereoHead):
    def __init__(self, in_channels, disp_range, alpha, normalize, learn_resize=False,  losses=None, **kwargs):
        '''
        description: 
        return: {*}
        '''        
        super(PSMNetHead48UnionelevelPostResize, self).__init__(in_channels, disp_range, alpha, normalize,  losses, **kwargs)
        self.in_channels = in_channels
        self.disp_range = disp_range 
        self.start_disp = disp_range[0]
        self.max_disp = disp_range[1]
...
@HEADS.register_module()
class PSMNetHead48Unionelevelv2(BaseStereoHead):
    def __init__(self, in_channels, disp_range, alpha, normalize, FPNhourglass=False, adabins="pre-define", learn_resize=None, losses=None, **kwargs):
        ...
        return: {*}
        '''        
        super(PSMNetHead48Unionelevelv2, self).__init__(in_channels, disp_range, alpha, normalize, losses, **kwargs)
        self.in_channels = in_channels
        self.learn_resize = learn_resize
