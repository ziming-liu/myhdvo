...
from ..backbones.resnet3d import BasicBlock3d,Bottleneck3d,MyResNet3dLayer
 

@HEADS.register_module()
class PyramidStereoHead2AdabinLight(nn.Module):
    def __init__(self, in_channels, max_disp, alpha, normalize, latent_dims=128, 
        losses=None, learn_resize=None, detach_mono_grad=False, 
        mask_size=4, adabins='pixel-wise', 
        ...
        return: {*}
        '''        
        super(PyramidStereoHead2AdabinLight, self).__init__()
        ...
        self.stage1 = MyResNet3dLayer( block=BasicBlock3d,
                 stage_block=2,
                 ...
                 temporal_stride=1,
                 dilation=1,)
        self.stage2 = MyResNet3dLayer(block=BasicBlock3d,
                 stage_block=2,
        ...
        self.prob_conv = nn.Conv3d(self.latent_dims, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.adabins_conv = nn.Conv3d(self.latent_dims, 1, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def cost_matcher(self, raw_costs):
        ...
        s1 = self.stage1(raw_costs)
        laten_s1 = self.l_conv1(s1)
        s2 = self.stage2(s1)
        laten_s2 = self.l_conv2(s2)

        up_laten_s2 = F.interpolate(laten_s2, size=(laten_s2.shape[2], 2*laten_s2.shape[3], 2*laten_s2.shape[4]),\
                                    mode="trilinear", align_corners=False)
        sum_laten_s1 = laten_s1 + up_laten_s2

        up_sum_laten_s1 = F.interpolate(sum_laten_s1, size=(sum_laten_s1.shape[2], 2*sum_laten_s1.shape[3], 2*sum_laten_s1.shape[4]), \
                                    mode="trilinear", align_corners=False)
        
        up_sum_laten_s1 = self.fpn_conv(up_sum_laten_s1)

        prob = self.prob_conv(up_sum_laten_s1).squeeze(1)

        adabins =  self.adabins_conv(up_sum_laten_s1).squeeze(1)
    ...
    def fast_cat_fms(self, reference_fm, target_fm, bins, y_bins=None):
        ...
        #return concat_fm
        return concat_reference_fm + concat_target_fm
...
@HEADS.register_module()
class PyramidStereoHead2AdabinLightLargeRange(PyramidStereoHead2AdabinLight):
