...
LastEditors: Ziming
LastEditTime: 2022-08-02 00:59:31
...
class ZNCC(nn.Module):
    ...
    def __init__(self,
        ...
        pad_size = (int((kernel_size[0]-1)/2), int((kernel_size[1]-1)/2))
        #conv2d = torch.nn.conv2d(3,3,(3,3),1,1,bias=False) 
        ...
        weight = nn.Parameter(data=kernel1, requires_grad=False).cuda()
        self.conv_mean = torch.nn.Conv2d(1, 1, kernel_size=kernel_size, stride=(1,1), padding=pad_size, padding_mode='reflect', bias=False).cuda()
        self.conv_mean.weight.data = weight
    ...
    def zeros_normalized_cross_correlation(self, x, y, return_map, reduction='mean', eps=1e-8):
        ...
        x_mean[:,0:1,:,:] = self.conv_mean(x[:,0:1,:,:]) #F.conv2d(x[:,0:1,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')
        x_mean[:,1:2,:,:] = self.conv_mean(x[:,1:2,:,:]) # F.conv2d(x[:,1:2,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')
        x_mean[:,2:3,:,:] = self.conv_mean(x[:,2:3,:,:]) # F.conv2d(x[:,2:3,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')

        
        #print("x\n",x, "\n xmean3x3\n",x_mean) 
        y_mean = torch.zeros(y.shape).type_as(y)
        y_mean[:,0:1,:,:] = self.conv_mean(y[:,0:1,:,:]) # F.conv2d(y[:,0:1,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')
        y_mean[:,1:2,:,:] = self.conv_mean(y[:,1:2,:,:]) # F.conv2d(y[:,1:2,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')
        y_mean[:,2:3,:,:] = self.conv_mean(y[:,2:3,:,:]) # F.conv2d(y[:,2:3,:,:], weight, stride=1, padding=pad_size, padding_mode='reflect')
        ...
        #conv_sum = torch.nn.Conv2d(1, 1, kernel_size=kernel_size, stride=(1,1), padding=pad_size, padding_mode='reflect', bias=False).to(x.device)
        ...
        #avg_xx_patch_sum = (1/(kernel_size[0]*kernel_size[1])) * xx_patch_sum
