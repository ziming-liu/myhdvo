...
from zimingdepth.utils import get_root_logger
...
class PointRecuder(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0,
        ...
        self.fc2 = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size,
...
class Cluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24,
        ...
        self.fc1 = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for similarity
        self.fc2 = nn.Conv2d(heads * head_dim, out_dim, kernel_size=1)  # for projecting channel number
        self.fc_v = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for value
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,
        ...
        self.fc1 = nn.Conv2d(in_features, hidden_features, kernel_size=1) #nn.Linear(in_features, hidden_features)
        self.act = act_layer()
        self.fc2 = nn.Conv2d(hidden_features, out_features, kernel_size=1) #nn.Linear(hidden_features, out_features)
...
class ContextCluster(nn.Module):
    ...
    out_indices: The contextcluster use a different out_indices, [0,2,4,6], there are point reducers.
    ...
    def __init__(self, layers, embed_dims=None, num_stages=4, out_indices=[0,2,4,6],
    ...
    def forward_embeddings(self, x):
        ...
        fea_pos = torch.stack(torch.meshgrid(range_w, range_h, ), dim=-1).float() # only support torch > 1.10 # indexing='ij'
