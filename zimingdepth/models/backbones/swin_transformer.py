    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            layers += [BasicLayer(
                ...
                input_resolution=(self.patch_grid[0] // (2 ** i), self.patch_grid[1] // (2 ** i)),#(self.patch_grid[0], self.patch_grid[1]),
                depth=depths[i],
...
class StereoSwinTransformer(nn.Module):
    def __init__(self, pretrained=None, **kwargs):
        super().__init__()
        self.transformer = _create_swin_transformer('swin_s3_tiny_224', pretrained=pretrained, **kwargs)

    def forward(self, *input):
        left_img, right_img = input
        print("input >> ", left_img.shape)
        left_feat = self.transformer.forward_features(left_img)
        print("final >> ", left_feat.shape)
        right_feat = self.transformer.forward_features(right_img)
