...
LastEditTime: 2023-03-23 18:44:31
...
class PyramidStereoSceneFlow2_1MonosLeft5(nn.Module):
    """
    Base depth method. 
    ...
    def __init__(self, backbone, neck, disp_head, mono_head, max_disp, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoSceneFlow2_1MonosLeft5, self).__init__()
    ...
    def extract_disp(self, left_img, right_img, **kwargs):
        ...
        if self.mono_head is not None:
            ...
            out.append(torch.zeros((B,1,H,W),device=cur_disps.device))
        # 1/16
        ...
        y_base, x_base =  y_base.unsqueeze(0).repeat(B,1,1).to(cur_disps.device), \
                         x_base.unsqueeze(0).repeat(B,1,1).to(cur_disps.device)
        
        for i in range(self.iters//2):
            cur_disps = cur_disps.detach()
            # Apply shift in X direction
            x_shifts = -cur_disps[:, 0, :, :]  # Disparity is passed in NCHW format with 1 channel
            ...
            warped_right = F.grid_sample(latent_feats[-1][B:,...], flow_field, mode='bilinear',padding_mode="zeros",align_corners=False)
            stereo_feat = [latent_feats[-1][:B,...], warped_right]
            ...
            cur_disps = cur_disps_list[0]
            #for cur_disps in cur_disps_list:
            out.append(out[-1] + F.interpolate(4 * self.convex_upsample(cur_disps, mask1, self.mask_size), scale_factor=4, mode="bilinear"))
        ...
        for i in range(self.iters//2):
            ...
            flow_field = (flow_field-0.5)*2.0
            # In grid_sample coordinates are assumed to be between -1 and 1
            warped_right = F.grid_sample(latent_feats[-2][B:,...], flow_field, mode='bilinear',padding_mode="zeros",align_corners=False)
            stereo_feat = [latent_feats[-2][:B,...], warped_right]
            ...
            #for cur_disps in cur_disps_list:
            out.append(out[-1] + F.interpolate(2*self.convex_upsample(cur_disps, mask2, self.mask_size), scale_factor=2, mode="bilinear"))
        ...
        for i in range(self.iters):
            ...
            flow_field = (flow_field-0.5)*2.0
            # In grid_sample coordinates are assumed to be between -1 and 1
            warped_right = F.grid_sample(latent_feats[-3][B:,...], flow_field, mode='bilinear',padding_mode="zeros",align_corners=False)
            stereo_feat = [latent_feats[-3][:B,...], warped_right]
            ...
            #for cur_disps in cur_disps_list:
            out.append(out[-1] + self.convex_upsample(cur_disps, mask3, 4))        
