...
class PyramidStereoSceneFlow2(nn.Module):
    """
    Base depth method. 
    ...
    def __init__(self, backbone, neck, disp_head, mono_head, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoSceneFlow2, self).__init__()
    ...
    def _init_weights_mask(self, ):
        ...
        self.conv_offset_16 = nn.Conv2d(
            self.in_channels, self.search_ranges[0], kernel_size=3, stride=1, padding=1
        )
        self.conv_offset_8 = nn.Conv2d(
            self.in_channels, self.search_ranges[1], kernel_size=3, stride=1, padding=1
        )
        self.range_16 = 1
        self.range_8 = 1

        # adaptive bins 
        self.conv_adabins1 = nn.Sequential(
            nn.Conv2d(self.in_channels, 128, 3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, self.search_ranges[0], 1, padding=0),
            
        )
        self.conv_adabins2 = nn.Sequential(
            nn.Conv2d(self.in_channels, 128, 3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, self.search_ranges[1], 1, padding=0),
            
        )
        self.conv_adabins3 = nn.Sequential(
            nn.Conv2d(self.in_channels, 128, 3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, self.search_ranges[2], 1, padding=0),
    ...
    def extract_disp(self, left_img, right_img, **kwargs):
        ...
        for i in range(self.iters//2):
            ...
            cur_disps, adabins_feat = self.disp_head1(stereo_feat, bins.detach(), y_bins)
        ...
        for i in range(self.iters//2):
            ...
            #bins = torch.clip(bins, 0, search_range-1)
            cur_disps, adabins_feat = self.disp_head2(stereo_feat, bins.detach(), y_bins)
            cur_disps = cur_disps[0] if isinstance(cur_disps,(list,tuple)) else cur_disps
        ...
        for i in range(self.iters//2):
            ...
            #bins = torch.clip(bins, 0, )
            #bins = torch.clip(bins, 0, search_range-1)
            cur_disps, adabins_feat = self.disp_head3(stereo_feat, bins.detach())
            cur_disps = cur_disps[0] if isinstance(cur_disps,(list,tuple)) else cur_disps
            mask3 = 0.25 * self.mask3(latent_feats[-3][:B,...])
            left_disps_3_x4 = self.convex_upsample(cur_disps, mask3, 4)
            #left_disps_3_x4 = F.interpolate(left_disps_3, scale_factor=4, mode="bilinear", align_corners=False)
            out.append(left_disps_3_x4)        
...
@STEREO_PREDICTOR.register_module()
class PyramidStereoSceneFlow2Unsup(PyramidStereoSceneFlow2):
...
@STEREO_PREDICTOR.register_module()
class PyramidStereoSceneFlow2v2(PyramidStereoSceneFlow2):
