...
class PixelNetSlowFast(nn.Module):
    """
    Base depth method. 

    """
    def __init__(self, backbone, losses, neck_dense, neck_sparse, alpha=1, normalize=True, sample_rate=4,
                  dense_channels=8, sparse_channels=64, cat_disp=48+12, max_disp=192, predict_format="depth", 
        ...
        """
        super(PixelNetSlowFast, self).__init__()
        ...
        self.linear = nn.Linear(cat_disp, max_disp)
    ...
    def forward_train(self, left_imgs, right_imgs, **kwargs):
        ...
        cost = torch.cat([cost_sparse, cost_dense], dim=2) # B 1 D H W
        cost = cost.permute(0,1,3,4,2) # B 1 H W D_cat
        cost = self.linear(cost)
        cost = cost.permute(0,1,4,2,3) # B 1 D H W
        #print(cost.shape)
