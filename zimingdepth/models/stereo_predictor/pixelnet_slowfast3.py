...
class PixelNetSlowFast3(nn.Module):
    """
    Base depth method. 

    """
    def __init__(self, backbone, losses, neck_dense, neck_sparse, alpha=1, normalize=True, sample_rate=4,
                  dense_channels=8, sparse_channels=64, dense_disp=192, sparse_disp=24, max_disp=192, predict_format="depth", 
        ...
        """
        super(PixelNetSlowFast3, self).__init__()
        self.sample_rate = sample_rate
        self.alpha = alpha
        self.normalize = normalize
        self.mask_left = mask_left
        self.dense_disp = dense_disp
        self.sparse_disp = sparse_disp
    ...
    def disp_predictor(self, final_costs, cat_cost=True,):
        ...
        for i in range(len(final_costs)):
            ...
            if self.normalize and not cat_cost:
            ...
            if cat_cost:
                dense_sample = torch.linspace(0, self.max_disp-self.max_disp//self.dense_disp, self.dense_disp, device=cost_volume.device)
                sparse_sample = torch.linspace(0, self.max_disp-self.max_disp//self.sparse_disp, self.sparse_disp, device=cost_volume.device)
                disp_sample = torch.cat([dense_sample.reshape(-1), sparse_sample.reshape((-1))], dim=0).reshape(1, -1, 1, 1).expand(B, -1, H, W)
                # compute disparity: (BatchSize, 1, Height, Width)
                disp_map = torch.sum(prob_volume * disp_sample, dim=1, keepdim=True) / 2 # average dense and sparse disparity
                ...
                disp_sample = disp_sample_pred_layer.reshape(1, -1, 1, 1).expand(B, -1, H, W)
                # compute disparity: (BatchSize, 1, Height, Width)
    ...
    def forward_train(self, left_imgs, right_imgs, **kwargs):
        ...
        cost_sparse, cost_dense = self.neck_sparse(feature_sparse), self.neck_dense(feature_dense) # B 256 D/sample_rate H/4 W/4 
        cost_sparse, cost_dense = self.classify_sparse(cost_sparse[0]), self.classify_dense(cost_dense[0]) # 256 -> 1 channel
        cost_sparse, cost_dense = F.softmax(cost_sparse*self.alpha, dim=2), F.softmax(cost_dense*self.alpha, dim=2) # softmax D dim separately 
        cost = torch.cat([cost_dense, cost_sparse, ], dim=2) # B 1 D+D/alpha H/4 W/4
        
        #print(cost.shape)
        cost_full = F.interpolate(cost.squeeze(1), scale_factor=(4,4), mode='bilinear') # B D1+D2 H W
