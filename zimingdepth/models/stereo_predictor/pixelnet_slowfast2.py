...
class PixelNetSlowFast2(nn.Module):
    """
    Base depth method. 

    """
    def __init__(self, backbone, losses, neck_dense, neck_sparse, alpha=1, normalize=True, sample_rate=4,
                  dense_channels=8, sparse_channels=64, sparse_disp=24, dense_disp=192, max_disp=192, predict_format="depth", 
                  pretrained=None, mask_left=False, agg_mode="add", **kwargs):
        """
        disp_l1_loss e.g.
        [dict(
        ...
        """
        super(PixelNetSlowFast2, self).__init__()
        ...
        self.sparse_linear = nn.Linear(sparse_disp, max_disp)
        self.dense_linear = nn.Linear(dense_disp, max_disp)
        if agg_mode == "stack":
            self.conv_stack = nn.Conv3d(2, 1, kernel_size=1, padding=0)
    ...
    def forward_train(self, left_imgs, right_imgs, **kwargs):
        ...
        cost_sparse = cost_sparse.permute(0,1,3,4,2) # B 1 H W D_sparse
        cost_sparse = self.sparse_linear(cost_sparse)
        cost_sparse = cost_sparse.permute(0,1,4,2,3) # B 1 D H W

        cost_dense = cost_dense.permute(0,1,3,4,2) # B 1 H W D_dense
        cost_dense = self.dense_linear(cost_dense)
        cost_dense = cost_dense.permute(0,1,4,2,3) # B 1 D H W
        if self.agg_mode =="add":
            cost = cost_sparse + cost_dense # B 1 D H W
        elif self.agg_mode == "stack":
            cost = torch.cat([cost_sparse, cost_dense], dim=1) # B 2 D H W
            cost = self.conv_stack(cost) # B 1 D H W

        #print(cost.shape)
        cost_full = F.interpolate(cost, scale_factor=(1,4,4), mode='trilinear').squeeze(1) # B D H W
        assert cost_full.shape[1] == self.max_disp
