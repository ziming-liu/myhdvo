...
LastEditTime: 2023-04-18 11:16:14
'''
import torch
...
class IGEVStereo(BaseStereo):
    def __init__(self, backbone, args, test_iters=32, **kwargs):
    ...
    def forward_train(self, image1, image2, iters=12, flow_init=None, test_mode=False, **kwargs):
    ...
    def forward_test(self, image1, image2, **kwargs):
        ...
        if self.args.val_init_disp:
            outputs[0] = init_disp.detach().cpu().numpy()
        else:
            outputs[0] = disp_up.detach().cpu().numpy()
        
        outputs[1] = kwargs['left_disps'].cpu().numpy()
        return outputs
