...
LastEditTime: 2023-03-27 16:01:07
...
class PyramidStereoSceneFlow2_1MonosLeft8Fixrange(nn.Module):
    """
    Base depth method. 
    ...
    def __init__(self, backbone, neck, disp_head, mono_head, max_disp, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoSceneFlow2_1MonosLeft8Fixrange, self).__init__()
    ...
    def extract_disp(self, left_img, right_img, **kwargs):
        ...
        if self.mono_head is not None:
            mask0 = 0.25 * self.mask0(feats[-1][:B,...])
            mono_disp = self.mono_head(feats[-1][:B,...]) # 1/32 resolution 
            mono_disp = torch.clip(mono_disp, 0, self.max_disp//32)
            ...
            scale = latent_feats[-1].shape[-1] / disp_convex.shape[-1]
