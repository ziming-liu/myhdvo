...
LastEditTime: 2023-03-27 23:39:53
...
class PyramidStereoSceneFlow2_1MonosLeft9(nn.Module):
    """
    Base depth method. 
    ...
    def __init__(self, backbone, neck, disp_head, mono_head, max_disp, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoSceneFlow2_1MonosLeft9, self).__init__()
        ...
        if neck is not None:
            ...
            self.mono_neck = build_neck(neck)
        self.neck_cfg = neck
    ...
    def extract_disp(self, left_img, right_img, **kwargs):
        ...
        mono_feats = self.mono_neck(raw_feats)
        latent_feats = []
        ...
        if self.mono_head is not None:
            mask0 = 0.25 * self.mask0(mono_feats[0][:B,...])
            mono_disp = self.mono_head(mono_feats[0][:B,...]) # 1/4 resolution 
            mono_disp = torch.clip(mono_disp, 0, self.max_disp//4)
            ...
            cur_disps = self.search_ranges[0]/2 * torch.ones((B,1,H//16,W//16),device=imgs.device)
        # 1/16
        ...
        for i in range(self.iters//2):
            ...
            cur_disps = torch.clip(cur_disps + delta_cur_disps_list[0], 0, self.max_disp//16)
        ...
        for i in range(self.iters//2):
            ...
            assert len(delta_cur_disps_list)==1
            cur_disps = torch.clip(cur_disps + delta_cur_disps_list[0], 0, self.max_disp//8)
        ...
        for i in range(self.iters):
            ...
            assert len(delta_cur_disps_list)==1
            cur_disps = torch.clip(cur_disps + delta_cur_disps_list[0], 0, self.max_disp//4)
    ...
    def _do_test(self, left_imgs, right_imgs,frame_dir,**kwargs):
        ...
        if self.output_branch == "stereo":
            outputs[0] = torch.clip(stereo_disp,0,self.max_disp).detach().cpu().numpy()
        else:
