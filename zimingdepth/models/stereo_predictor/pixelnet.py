...
class PixelNet(nn.Module):
    ...
    """
    def __init__(self, backbone, losses, neck=None, alpha=1, normalize=True, sample_rate=4, max_disp=192, predict_format="depth", pretrained=None, **kwargs):
        """
        disp_l1_loss e.g.
        [dict(
    ...
    def _do_test(self, left_imgs, right_imgs,**kwargs):
        ...
        disp = self.disp_predictor([cost_full])[0]
        #t2 = time.time()
        #print(f"build image volume {t1-t0} network {t2-t1}, total {t2-t0} ")
        results["disps"] = disp
        results["right_disps"] = None
