...
Author: Ziming Liu
Date: 2023-03-22 11:54:58
LastEditors: Ziming Liu
LastEditTime: 2023-03-22 15:52:51
Note: v1 code, only keep the simple multi-level structure, without mono initialization. 
'''
import torch
...
class PyramidStereoV1(nn.Module):
    """
    Base depth method. 

    """
    def __init__(self, backbone, neck, disp_head1, disp_head2, disp_head3, mono_head, max_disp, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoV1, self).__init__()
        ...
        for i in range(self.iters//2):      
            self.disp_head1.append(build_head(disp_head1))
        for i in range(self.iters//2):      
            self.disp_head2.append(build_head(disp_head2))
        for i in range(self.iters):      
            self.disp_head3.append(build_head(disp_head3))
    ...
    def extract_disp(self, left_img, right_img, **kwargs):
        ...
        #cur_disps = torch.zeros((B,1,H//16,W//16), device=stereo_feat.device)
        B,_,H,W = stereo_feat[0].shape
        mask1 = 0.25 * self.mask1(latent_feats[-1][:B,...])
        for i in range(self.iters//2):
             
            cur_disps_list = self.disp_head1[i](stereo_feat, )
            cur_disps_list = [ cur_disps_list[k] for k in range(len(cur_disps_list)-1, -1, -1)]
            if i!=0:last_disp  = out[-1]
            for cur_disps in cur_disps_list:
                x_up = self.convex_upsample(cur_disps, mask1, 4)
                if i==0:
                    out.append(x_up)
                else:
                    out.append(x_up + last_disp)
        ...
        for i in range(self.iters//2):
            ...
            cur_disps_list = self.disp_head2[i](stereo_feat, )
            cur_disps_list = [ cur_disps_list[k] for k in range(len(cur_disps_list)-1, -1, -1)]
            last_disp = out[-1]
            ...
            for cur_disps in cur_disps_list:
                x_up =  self.convex_upsample(cur_disps, mask2, 2)
                out.append(x_up + last_disp)
        ...
        B,_,H,W = stereo_feat[0].shape
        for i in range(self.iters):
            cur_disps_list = self.disp_head3[i](stereo_feat )
            cur_disps_list = [ cur_disps_list[k] for k in range(len(cur_disps_list)-1, -1, -1)]
            last_disp = out[-1]
            for cur_disps in cur_disps_list:
                #x_up = self.convex_upsample(cur_disps, mask3, 4)
                out.append(x_up + last_disp)        
