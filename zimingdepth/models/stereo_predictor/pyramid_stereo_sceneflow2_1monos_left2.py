...
Author: Ziming Liu
Date: 2023-03-22 23:17:36
LastEditors: Ziming Liu
LastEditTime: 2023-03-22 23:32:17
make each head to have individual parameters.
'''
import torch
...
class PyramidStereoSceneFlow2_1MonosLeft2(nn.Module):
    """
    Base depth method. 
    ...
    def __init__(self, backbone, neck, disp_head, mono_head, max_disp, in_channels=128,
      ...
      iters=4, **kwargs):
        ...
        """
        super(PyramidStereoSceneFlow2_1MonosLeft2, self).__init__()
        ...
        for i in range(self.iters//2):
            self.disp_head1.append(build_head(disp_head))
        self.disp_head2 = nn.ModuleList()
        for i in range(self.iters//2):
            self.disp_head2.append(build_head(disp_head))
        self.disp_head3 = nn.ModuleList()
        for i in range(self.iters):
            self.disp_head3.append(build_head(disp_head))
        if neck is not None:
