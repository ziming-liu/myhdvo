...
class EvalHook(Hook):
    ...
    def _do_evaluate(self, runner):
        ...
        from zimingdepth.apis import single_gpu_test
        results = single_gpu_test(runner.model, self.dataloader)
        key_score = self.evaluate(runner, results, self.metrics)
        if self.save_best:
    ...
    def evaluate(self, runner, results, metrics):
        ...
        eval_res = self.dataloader.dataset.evaluate(
            results, metrics=metrics, logger=runner.logger, **self.eval_kwargs)
...
class DistEvalHook(EvalHook):
    ...
    def _do_evaluate(self, runner):
        ...
        from zimingdepth.apis import multi_gpu_test
        tmpdir = self.tmpdir
        if tmpdir is None:
