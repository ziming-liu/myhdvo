...
def getKITTI2015Metas(root, Type, mode, is_full=True):
    ...
    imageNames = [img for img in os.listdir(osp.join(root, Type, 'colored_0')) if img.find('_10') > -1]
    imageNames.sort()
    if Type == 'training':
        assert len(imageNames) == 194
    elif Type == 'testing':
        assert len(imageNames) == 195
    for imageName in imageNames:
        meta = dict(
            left_image_path=osp.join(
                Type, 'colored_0', imageName
            ),
            right_image_path=osp.join(
                Type, 'colored_1', imageName
            ),
            left_disp_map_path=osp.join(
                Type, 'disp_occ', imageName
            ) if Type == 'training' else None,  # testing dataset has no ground truth left disparity map
            right_disp_map_path= None,  # testing dataset has no ground truth left disparity map
        )
        Metas.append(meta)

    # if not is_full, split the 194 images into training and evaluating
    eval_list = [1, 3, 6, 20, 26, 35, 38, 41, 43, 44,
                 49, 60, 67, 70, 81, 84, 89, 97, 109, 119,
                 122, 123, 129, 130, 132, 134, 141, 144, 152, 158,
                 159, 165, 171, 174, 179, 182, 184, 186, 187, ]
...
KITTI-2015 Dataset contains:
      194   training samples
      194   validation samples
      195   testing samples
Save to /home/ziliu/mydata/kittistereo2012/annotations/full_train.json
Save to /home/ziliu/mydata/kittistereo2012/annotations/full_eval.json
Save to /home/ziliu/mydata/kittistereo2012/annotations/full_test.json

KITTI-2015 Dataset contains:
      155   training samples
       39   validation samples
      195   testing samples
Save to /home/ziliu/mydata/kittistereo2012/annotations/split_train.json
Save to /home/ziliu/mydata/kittistereo2012/annotations/split_eval.json
Save to /home/ziliu/mydata/kittistereo2012/annotations/split_test.json
"""