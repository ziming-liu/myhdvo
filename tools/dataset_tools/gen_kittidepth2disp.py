...
def getKITTIMetas(root, Type, mode, is_full=True, ):
    r"""
    Arguments:
...
def build_annoFile(root, save_annotation_root, is_full=True, ):
    ...
    trainMetas = getKITTIMetas(root, 'training', mode='training', is_full=is_full, )
    evalMetas = getKITTIMetas(root, 'training', mode='evaluating', is_full=is_full, )
    testMetas = getKITTIMetas(root, 'testing', mode='testing', is_full=True, )

    check(root, trainMetas)
    check(root, evalMetas)
    check(root, testMetas)

    info_str = 'KITTI-depth2disp Dataset contains:\n' \
               '    {:5d}   training samples \n' \
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="KITTI depth2disp Data PreProcess.")
    parser.add_argument(
        "--data-root",
        default=None,
        help="root of data",
