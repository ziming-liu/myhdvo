...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="20",
        help=" seq test time training size"
    )
    parser.add_argument(
        '--test_range',
        type=str,
...
def main():
    ...
    base_model_params =  copy.deepcopy(model.state_dict())
    #model.eval()
    #results = []
    results = [[] for _ in range(10)] # depth, pose, seq_dir
    dataset = data_loader.dataset
    prog_bar = mmcv.ProgressBar(len(dataset))
    model.load_state_dict(base_model_params)
    ...
    for idx, data in enumerate(data_loader):
        ...
        optimizer.step() 
        t1 = time.time()
        print("TTT time: {}".format(t1-t0))
        model.eval()
        with torch.no_grad():
            #result = model.val_step(data, None)
            left_imgs = data['left_imgs']
            right_imgs = data['right_imgs'] if "right_imgs" in data else None
            
            aux_info = {}
            keys = list(data.keys())
            keys.remove('left_imgs')
            if "right_imgs" in data:
                keys.remove('right_imgs')
            for item in keys:
                aux_info[item] = data[item]
            result = model(left_imgs, right_imgs, return_loss=False, **aux_info)
        t2 = time.time()
        print("get pose time: {}".format(t2-t1))
        num_output = len(result)
        #print(f"num output {num_output}")
        ...
        if not args.seq_optim:
            #load_checkpoint(model, "./temp.pth",)
            print("load dict at s {}".format(idx))
            model.load_state_dict(base_model_params)
            optimizer = optim.AdamW(model.parameters(), args.lr)
