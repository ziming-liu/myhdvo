...
###
 ...
 # @LastEditTime: 2023-04-04 10:07:03
 # @Description: ...
...
#OAR --name  psmnet_small_b8_densesparse2
...
#PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
 OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=1   \
    tools/train.py configs/psmnet/psmnet_small_b8_densesparse2.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=1 --master_port=12200 \
  tools/test.py configs/psmnet/psmnet_small_b8_densesparse2.py \
   work_dirs/psmnet_small_b8_densesparse2/iter_48000.pth   \
