...
LastEditTime: 2023-03-27 13:06:17
...
_base_ = [
    '../_base_/datasets/kittistereo2012_random384x512.py', '../_base_/iters_runtime.py',
    '../_base_/schedules/schedule_adamW_3k.py'
]
...
work_dir = "work_dirs/pyramid_base_setting_cascade8onekitti2012dirvingpretrain"
load_from =  "work_dirs/pyramid_base_setting_cascade8onedriving200k/iter_200000.pth"
# load pretrain from driving stereo 4 iters 

# optimizer 
# adamW has faster training time and lower errors than adam
optimizer = dict(type='AdamW', lr=1e-5, )
#optimizer_config = dict(type="GradientCumulativeOptimizerHook", cumulative_iters=4)
optimizer_config = dict() #dict(type="GradientCumulativeFp16OptimizerHook", cumulative_iters=4, loss_scale="dynamic",
                   #     grad_clip=None )

# learning policy, one cycle is faster to concerge with similar acc. 
lr_config = dict(policy='OneCycle',
    by_epoch=False, max_lr=1e-5, total_steps=3000,) 
# runtime settings
runner = dict(type='IterBasedRunner', max_iters= 3000)
total_epochs=3000
checkpoint_config = dict(by_epoch=False, interval=1000)
evaluation = dict(interval=1000, metrics='D1', )
