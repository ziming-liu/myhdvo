...
###
 ...
 # @LastEditTime: 2023-04-10 16:51:39
 # @Description: ...
...
#OAR --name  pixelnet50_small_b8_slowonly
...
#PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
 OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2   \
    tools/train.py configs/pixelnet/pixelnet50_small_b8_slowonly.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_port=12200 \
  tools/test.py configs/pixelnet/pixelnet50_small_b8_slowonly.py \
   work_dirs/pixelnet50_small_b8_slowonly/iter_48000.pth   \
