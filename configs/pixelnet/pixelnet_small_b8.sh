...
###
 ...
 # @LastEditTime: 2023-04-06 02:06:37
 # @Description: ...
...
#PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
 OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2   \
    tools/train.py configs/pixelnet/pixelnet_small_b8.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=4 --master_port=12200 \
  tools/test.py configs/pixelnet/pixelnet_small_b8.py \
   work_dirs/pixelnet_small_b8/iter_48000.pth   \
