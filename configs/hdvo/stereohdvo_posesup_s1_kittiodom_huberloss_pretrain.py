...
LastEditTime: 2024-02-27 23:44:33
'''
_base_ = [
    '../_base_/datasets/kittidepth_odometry_rgb_crop_testset.py', 
]
...
model=dict(
    ...
    depth_net=dict(
        type="PSMNet",
        # pretrained="work_dirs/pretrained_sceneflow.tar",
        subnetwork=True,
        backbone=dict(
            type="PSMNetSingle",
            pretrained="work_dirs/pretrained_sceneflow.tar",
    ...
    ddvo_head=dict(
        type="PoseDDVOHead",
        ddvo=dict(type="DirectVO_OpenRox", ifmask=1, disp_log=0, ifrobust=1,),
         photo_loss=dict(type='HUBERLoss', ratio=0.85), #dict(type='HUBERLoss', ratio=0.85), # dict(type='ZNCCLoss', ratio=0.85), #
        struct_loss = dict(type='SSIMLoss', ratio=0.15),
        grid_sample_type="pytorch", 
        padding_mode="zeros",),
...
work_dir = 'work_dirs/stereohdvo_posesup_s1_kittiodom_huberloss_pretrain'