...
###
 ...
 # @LastEditTime: 2023-03-23 18:35:16
 # @Description: ...
...
#OAR --name  pyramid_base_setting_cascade6
...
OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=3 --master_port=16973 \
    tools/train.py configs/pyramid_stereo/pyramid_base_setting_cascade6.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

torchrun --standalone --nnodes=1 --nproc_per_node=3 --master_port=12200 \
  tools/test.py configs/pyramid_stereo/pyramid_base_setting_cascade6.py \
   work_dirs/pyramid_base_setting_cascade6/epoch_2.pth   \
