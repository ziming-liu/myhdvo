...
###
 ...
 # @LastEditTime: 2023-03-21 17:55:50
 # @Description: ...
...
#OAR --name  pyramid_base_setting_cascade_lightconv
...
#PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
 OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_port=16973 \
    tools/train.py configs/pyramid_stereo/pyramid_base_setting_cascade_lightconv.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_port=12200 \
  tools/test.py configs/pyramid_stereo/pyramid_base_setting_cascade_lightconv.py \
   work_dirs/pyramid_base_setting_cascade_lightconv/iter_2.pth   \
