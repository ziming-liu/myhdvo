...
###
 ...
 # @LastEditTime: 2023-03-24 16:23:08
 # @Description: ...
...
#OAR --name  pyramid_base_setting_cascade7one
...
OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=2 --master_port=16973 \
    tools/train.py configs/pyramid_stereo/pyramid_base_setting_cascade7one.py \
      --launcher pytorch  --validate
 
torchrun --standalone --nnodes=1 --nproc_per_node=3 --master_port=12200 \
  tools/test.py configs/pyramid_stereo/pyramid_base_setting_cascade7one.py \
   work_dirs/pyramid_base_setting_cascade7one/epoch_1.pth   \
