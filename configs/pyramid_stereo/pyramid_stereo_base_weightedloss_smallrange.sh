...
###
 ...
 # @LastEditTime: 2023-03-13 00:49:51
 # @Description: ...
...
#OAR --name  pyramid_stereo_base_weightedloss_smallrange
...
#PYTHONPATH="$(dirname $0)/..":$PYTHONPATH \
 OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=3 --master_port=16773 \
    tools/train.py configs/pyramid_stereo/pyramid_stereo_base_weightedloss_smallrange.py \
      --launcher pytorch  --validate
# Any arguments from the third one are captured by ${@:3}

OMP_NUM_THREADS=12 torchrun --standalone --nnodes=1 --nproc_per_node=3 --master_port=12200 \
  tools/test.py configs/pyramid_stereo/pyramid_stereo_base_weightedloss_smallrange.py \
   work_dirs/pyramid_stereo_base_weightedloss_smallrange/epoch_16.pth   \
