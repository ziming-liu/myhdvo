...
data_root = osp.join(root2, 'drivingstereo')
annfile_root = osp.join(root2, "drivingstereo")
...
dataset_type = 'DrivingStereoDataset'
...
train_pipeline = [
    ...
    dict(type='KITTIKBCrop', keys=['left_imgs', 'right_imgs', 'left_disps',], crop_size=[320,855]),
    ...
    #dict(type='Augmentor', image_height=crop_size[0],
    #                        image_width=crop_size[1],
    #                        max_disp=192,
    #                        scale_min=0.6,
    #                        scale_max=1.0,
    #                        seed=0,),
...
val_pipeline = [
    ...
    #dict(type='Flip', flip_ratio=0),
    dict(type='StereoNormalize', **img_norm_cfg),
    dict(type='StereoPad', size=(448,896), pad_val=0, disp_pad_val=0), # 400,879
    dict(type='StereoFormatShape', input_format='NCHW', keys=['left_imgs','right_imgs', 'left_disps',   ]),
    dict(type='Collect', keys=['left_imgs','right_imgs', 'left_disps',    ], meta_keys=[ ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs', 'left_disps',    ])
]
test_pipeline = [
    ...
    #dict(type='Flip', flip_ratio=0),
    dict(type='StereoNormalize', **img_norm_cfg),
    dict(type='StereoPad', size=(448,896), pad_val=0, disp_pad_val=0),
    dict(type='StereoFormatShape', input_format='NCHW', keys=['left_imgs','right_imgs', 'left_disps',   ]),
    dict(type='Collect', keys=['left_imgs','right_imgs', 'left_disps',  ], meta_keys=[ ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs',  'left_disps', ])
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "drivingstereo_half_train.json"),
    ...
    val=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "drivingstereo_half_test.json"),
        end_id=100,
        data_prefix=data_root,
        eval_modality="disparity",
        depth_scale_ratio=256,
    ...
    test=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "drivingstereo_half_test_2018-07-11-14-48-52.json"),
        #end_id=10,
        data_prefix=data_root,
        eval_modality="disparity",
        depth_scale_ratio=256,
