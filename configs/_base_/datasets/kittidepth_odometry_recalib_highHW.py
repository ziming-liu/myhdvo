"""
Save to /home/ziliu/mydata/kitti_odometry_recalib/kitti_odometry_recalib_train.json
1590it [00:00, 333864.50it/s]
1200it [00:00, 289029.79it/s]
Save to /home/ziliu/mydata/kitti_odometry_recalib/kitti_odometry_recalib_test_09.json
"""
...
data_root = root2
annfile_root = osp.join(root2, "kitti_odometry_recalib")
 

dataset_type = 'KITTIOdometryDataset'
 
img_norm_cfg = dict(mean=[96.3196,], std=[74.0446,], to_rgb=False)
...
train_pipeline = [
    dict(type='LoadStereoImages', views=["left", "right"], to_float32=False,
                 ...
                 imdecode_backend='cv2', to_rgb=False, to_gray=False), # let rgb to gray
...
val_pipeline = [
    dict(type='LoadStereoImages', to_float32=False,
                 color_type='grayscale',
                 file_client_args=dict(backend='disk'),
                 imdecode_backend='cv2', to_rgb=False, to_gray=False),
    #dict(type='LoadAnnotations', views=["left", ],  modalities=["disp"],),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_odometry_recalib_train.json"),
    ...
    val=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_odometry_recalib_test_09.json"),
        end_id=10,
        data_prefix=data_root,
        eval_modality="depth",
        depth_scale_ratio=256,
