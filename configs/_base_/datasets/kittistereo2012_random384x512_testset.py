...
data_root = osp.join(root2, 'kittistereo2012/data_stereo_flow')
annfile_root = osp.join(root2, "kittistereo2012/annotations")
...
val_pipeline = [
    ...
    dict(type='Collect', keys=['left_imgs','right_imgs',      ], meta_keys=['left_frame_paths']),
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root,"split_test.json"), #"full_test.json"),
        #end_id=10,
        data_prefix=data_root,
        eval_modality="disp",
        depth_scale_ratio=256,
