...
test_pipeline = [
   ...
   # dict(type='KITTIKBCrop', keys=['left_imgs', 'right_imgs', ],crop_size=[256,1216]), #crop_size=[256,1216]), #crop_size=[256,1216]),
    ...
    #dict(type='StereoPad', size=(416,1312), pad_val=0, disp_pad_val=0),
    #dict(type='StereoPad', size=crop_size, pad_val=0, disp_pad_val=0),
    dict(type='StereoFormatShape', input_format='NTCHW', keys=['left_imgs','right_imgs',  'raw_left_imgs','raw_right_imgs', 'left_depths'   ]),
    dict(type='Collect', keys=['left_imgs', 'right_imgs',  'raw_left_imgs','raw_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'  ], meta_keys=[  ]),
    dict(type='ToTensor', keys=['left_imgs',  'right_imgs',  'raw_left_imgs','raw_right_imgs',  'intrinsics', 'focal', 'baseline', 'pose'    ])
]
...
data = dict(
    ...
    train=dict(
        ...
        # Training on sequences 02, 06, 18 (dynamic3 experiment)
        ann_files=[
            osp.join(annfile_root, "dynamic3/scene01.json"),
            osp.join(annfile_root, "dynamic3/scene06.json"),
            osp.join(annfile_root, "dynamic3/scene18.json"),
        ],
    ...
    test=dict(
        ...
        # Multiple test sequences - Testing on sequences 01, 20 (dynamic3 experiment)
        test_sequences=[
            dict(
                ann_file=osp.join(annfile_root, "dynamic/scene02.json"),
                test_seq_id="Scene02_dynamic",
                scene="Scene02",
                variation="clone"
            ),
            dict(
                ann_file=osp.join(annfile_root, "dynamic/scene20.json"),
                test_seq_id="Scene20_dynamic",
                scene="Scene20",
                variation="clone"
            ),
        ]
