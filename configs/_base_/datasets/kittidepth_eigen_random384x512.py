...
train_pipeline = [
    ...
    dict(type='StereoFormatShape', input_format='NTCHW', keys=['left_imgs','right_imgs',  'gray_left_imgs', 'gray_right_imgs',     ]),
    dict(type='Collect', keys=['left_imgs','right_imgs', 'gray_left_imgs', 'gray_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'  ], meta_keys=[  ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs',  'gray_left_imgs', 'gray_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'    ])
]
val_pipeline = [
    dict(type='LoadStereoImages', to_float32=False,
                 color_type='color',
    ...
    dict(type='StereoFormatShape', input_format='NTCHW', keys=['left_imgs','right_imgs', 'gray_left_imgs', 'gray_right_imgs',   ]),
    dict(type='Collect', keys=['left_imgs','right_imgs', 'gray_left_imgs', 'gray_right_imgs',  'intrinsics', 'focal', 'baseline'    ], meta_keys=[ ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs',  'gray_left_imgs', 'gray_right_imgs', 'intrinsics', 'focal', 'baseline'    ])
]
test_pipeline = [
    dict(type='LoadStereoImages', to_float32=False,
                 color_type='color',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_depth_train_wt_pose.json"),
    ...
    val=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_depth_test.json"),
        #end_id=100,
        data_prefix=data_root,
        eval_modality="depth",
        depth_scale_ratio=256,
