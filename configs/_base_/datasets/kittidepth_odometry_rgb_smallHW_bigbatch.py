...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', views=["left", ],  modalities=["depth"],),
    dict(type='StereoResize2', img_scale=RESIZE_SIZE, keep_ratio=False, ratio_range=(1, 1.)),
    dict(type='KITTIKBCrop', keys=['left_imgs', 'right_imgs', 'left_depths'],crop_size=[256,1024]), #crop_size=[256,1216]),
    ...
    dict(type='StereoFormatShape', input_format='NTCHW', keys=['left_imgs','right_imgs', 'left_depths', 'raw_left_imgs','raw_right_imgs',   ]),
    dict(type='Collect', keys=['left_imgs','right_imgs',  'raw_left_imgs','raw_right_imgs', 'left_depths', 'intrinsics', 'focal', 'baseline', 'pose'  ], meta_keys=[ 'left_frame_paths','right_frame_paths' ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs', 'left_depths',  'raw_left_imgs','raw_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'    ])
]
val_pipeline = [
    ...
    dict(type='Collect', keys=['left_imgs','right_imgs',  'raw_left_imgs','raw_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'  ], meta_keys=[ 'left_frame_paths','right_frame_paths' ]),
...
test_pipeline = [
    dict(type='LoadStereoImages', to_float32=False,
                 ...
                 imdecode_backend='cv2', to_rgb=True, to_gray=False),
    dict(type='LoadAnnotations', views=["left", ],  modalities=["depth"],),
    #dict(type='KITTIKBCrop', keys=['left_imgs', 'right_imgs', 'left_depths'],crop_size=[256,1216]), #crop_size=[256,1216]),
    #dict(type='Resize', scale=(650, 200)), # w, h 
    #dict(type='StereoCenterCrop', crop_size=(0.40810811,0.99189189,0.03594771, 0.96405229)),
    #dict(type='StereoResize', scale=RESIZE_SIZE, keep_ratio=False),
    dict(type='StereoResize2', img_scale=RESIZE_SIZE, keep_ratio=False, ratio_range=(1, 1.)),
    ...
    #dict(type='StereoPad', size=crop_size, pad_val=0, disp_pad_val=0),
    dict(type='StereoFormatShape', input_format='NTCHW', keys=['left_imgs','right_imgs', 'left_depths', 'raw_left_imgs','raw_right_imgs',   ]),
    dict(type='Collect', keys=['left_imgs','right_imgs', 'left_depths', 'raw_left_imgs','raw_right_imgs', 'intrinsics', 'focal', 'baseline', 'pose'  ], meta_keys=['left_frame_paths','right_frame_paths'  ]),
    dict(type='ToTensor', keys=['left_imgs','right_imgs',  'left_depths', 'raw_left_imgs','raw_right_imgs',  'intrinsics', 'focal', 'baseline', 'pose'    ])
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_odometry_train.json"),
    ...
    test=dict(
        type=dataset_type,
        ann_file=osp.join(annfile_root, "kitti_odometry_test_10.json"),
        end_id=100,#1400,
        data_prefix=data_root,
        eval_modality="depth",
        depth_scale_ratio=256,
