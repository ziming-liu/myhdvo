...
)
{
   ...
   if(ifrobust==1){
      ...
      // Robust estimation with Huber  TODO
...
)
{
   ...
   {
      error = rox_odometry_dense_depthmap_prepare ( obj, Ic, ifrobust);
      ROX_ERROR_CHECK_TERMINATE ( error );
...
)
{
   ...
   {
      error = rox_odometry_dense_depthmap_prepare ( obj_left, image_left, ifrobust);
      ROX_ERROR_CHECK_TERMINATE ( error );

      error = rox_odometry_dense_depthmap_prepare ( obj_right, image_right, ifrobust);
      ROX_ERROR_CHECK_TERMINATE ( error );
